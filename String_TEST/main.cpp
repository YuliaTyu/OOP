...
public:
	...
	String(String&& other)// выполняет Shallow Copy - поверхностое копирование(копируется адрес)
	{
		this->size = other.size;
		this->str = other.str;
		other.size = 0;
	...
	// MoveAssignment : оператор присваивания-переноса выполняет Shaloow copy
